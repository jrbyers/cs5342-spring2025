...
def label_data(posts_df, labeled_csv='labeled_data.csv'):
    ...
    if os.path.exists(labeled_csv):
        ...
        print(f"Resuming from post #{len(labeled_df)}. {len(remaining_posts_df)} posts remaining to label.")
    else:
        # If no labeled CSV, randomize the posts for labeling
        remaining_posts_df = posts_df.sample(frac=1).reset_index(drop=True)
        print(f"Starting new labeling session. {len(remaining_posts_df)} posts to label.")

    # Prepare the list to store labeled data
    labeled_data = labeled_df.to_dict(orient='records') if os.path.exists(labeled_csv) else []
